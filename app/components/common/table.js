...
export default class TableBox extends Component {
    ...
    renderBody () {
        ...
        const body = [...list].map((tr, i) => {
            ...
            if(isExpand){
                ...
                const expandElement = (
                    <tr className={trName}><td colSpan={colSpan}>
    ...
    renderOperateIcon (item,id,i,rowIndex) {
        ...
        const tooltip = (<Tooltip id={'j_operate_'+operate+'_'+id}>{text}</Tooltip>);
        return (
    ...
    handleOnClick (event, rowIndex) {
        ...
        this.props.onTable({
            "operate": operate,
            "data": data,
            "rowIndex": target.getAttribute('data-rowIndex')
        });
    };
