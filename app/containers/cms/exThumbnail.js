...
export class ExThumbnailBox extends Component {
    ...
    getTpl() {
        ...
        let addIcon = '';
        if(addPic){
            addIcon = <Col xs={12} sm={4} md={3} lg={3} className="thumbnailBox hand" onClick={this.props.addPic}><i className="thumbnail fa fa-plus fa-5x center"></i></Col>
        }
        let isTip=false;
        ...
        return(<div className="clearfix">
            {[...list].map((item, i)=>{
                ...
                return (
                    <Col  xs={12} sm={4} md={3} lg={3} key={i} className="thumbnailBox">
                        {updatePicInfo &&
                            <div>
                        ...
                        <Thumbnail src={src && src[0] == '/' ? uploadPrefix + src : src} alt={alt} data-key={i} >
                            <Row className="mt-10 mb-10">
                                {sequenceTag}
                                {deletePic && <Button className="fr" onClick={this.deletePic.bind(this, i)}>删除</Button>}
                                <Upload className="fr"  {...uploadProps} onSuccess onChange={this.onUploadFile.bind(this, i, 'src')}>
                                    <Button>
