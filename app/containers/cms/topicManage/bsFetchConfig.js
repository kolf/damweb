			  	...
			  	"head": [{
			      	...
			      	customize (text, index) {
			        	if(index < 4){
			          		...
			          		return <div><Button className="btn-danger fr" shape="circle" icon="cross" onClick={_this.delFetch.bind(_this, index)} /><label>标签：</label><input value={text} onChange={_this.handleInputChange.bind(_this, "bsFetch", index)}/></div>;
			        	}
			      	}
			    },
			    {
			        	...
			        	if(index >= 3){
			          		return (<div>
			            		{[...contentArr].map((item, i) => {
			              			return <span key={i}><input style={{width:80}} value={item} onChange={_this.handleInputChange.bind(_this, "bsFetch", index, i)}/>
			              				<Button shape="circle" size="small" icon="minus" className="btn-danger" onClick={_this.delFetchContent.bind(_this, index, i)}></Button></span>;
			            		})}
			          		<Button size="small" shape="circle" type="primary" icon="plus" onClick={_this.addFetchItem.bind(_this, index)}></Button></div>)
			        	// }else if(index == 3){
			        	// 	return <div>
			         ...
			         //  		</div>
			        		...
			        		contentArr[1] = contentArr[1] || currentDate;
				          	...
				          	return <div><DatePicker style={{width:200}} value={contentArr[0]} format="yyyy-MM-dd HH:mm:ss" showTime={true} onChange={_this.handleTimeChange.bind(_this, 0)}/>-
				          				<DatePicker style={{width:200}} value={contentArr[1]} format="yyyy-MM-dd HH:mm:ss" showTime={true} onChange={_this.handleTimeChange.bind(_this, 1)}/></div>
			        	}
			      	}
			    },
			    {
			      ...
			      customize (text, index) {
			        if(!text){
			          ...
			          return <Select defaultValue={text} style={{width:150}} onSelect={(v)=>{_this.state.bsFetch.list[index].source = v;}}>
