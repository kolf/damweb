...
const uploadURL = 'http://123.56.31.128:8002/v1/upload';
...
export default class BasicInfo extends Component {
			  ...
			  "head": [
			    {
				      	...
				      	customize (text) {
				        	...
				        	return  <Select defaultValue={text} className="col-sm-8" onSelect={(v)=>{_this.state.thumbnail.list[0].live = v}}>
				...
				"head": [
				    {
				      	...
				      	customize (text) {
				        	...
				        	return  <Select defaultValue={text} className="col-sm-8" onSelect={(v)=>{_this.state.coverPic.list[0].column = v}}>
				      	...
				      	customize (text) {
				      		...
				      		if(categories instanceof Array){
				      			...
				      			categories.forEach((item, i)=>{
				      				optionArr.push(<Option value={item.id} key={i}>{item.name}</Option>)
				      			});
				      			return <Select defaultValue={text} className="col-sm-8" onSelect={(v)=>{_this.state.coverPic.list[0].categories = v}}>
				      					{optionArr}
				                	</Select>
				      		}
	...
	init () {
		...
		dispatch(getBasicInfo(params)).then((result) => {
			...
			this.state.thumbnail.list.push(result.apiResponse.thumbnail.list);
			this.state.coverPic.list.push(result.apiResponse.coverPic.list);
			//topicInfo
