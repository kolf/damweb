...
import {channelCfg, scrollList, picList, tagData, tagPicList, deletePic} 		  from "app/action_creators/cms_action_creator";
...
export default class CreativeFontpage extends Component {
	constructor (props) {
		...
		for(let i=0;i<12;i++)
		{
			if((0<=i&&i<4)||(i<12&&i>=7))
	        {
	        	tipArr.push("请上传宽/高比例为290/220的图")
	        	
	        }
	        else if(i<7&&i>=4)
	        {
	        	tipArr.push("请上传宽/高比例为390/220的图")
				...
				"idField": "id",
			  	"head": [
				    ...
				    },
				    {
				      	...
				      	"type": "customize",
				      	customize (text, i) {
				        	return <input style={{width:'100%'}} value={text} onChange={_this.updateTagData.bind(_this, i, 'title')}/>
				    	}
				    },
				    {
				      	"field": "link",
	...
	render () {
		const {crumbs, alert, topicInfo, banner, picListData, scrollList, selectedTag} = this.state;
		...
		return (<div>
				...
				<Tabs type="card" onChange={this.tabChange.bind(this)}>
					<Tabpane tab="焦点图设置" key="1">
						<FocusPic {...picListData} tipMessage={{isSame:1,message:["请上传宽/高比例为1000/416的图"]}} onUpdateData={this.updateData.bind(this,1000,416)} addData={this.addData.bind(this)} sortAble={true}/>
					</Tabpane>
					<Tabpane tab="热点活动" key="4">
						<FocusPic {...picListData} tipMessage={{isSame:1,message:["请上传宽/高比例为390/180的图"]}} onUpdateData={this.updateData.bind(this,390,180)} addData={this.addData.bind(this)} sortAble={true}/>
					</Tabpane>
					...
					<Tabpane tab="标签图片管理" key="5">
						标签：<Select value={selectedTag} style={{width:'200px', marginBottom:'15px'}} onSelect={this.handleOnSelectTag.bind(this)}>
							{scrollList.list.map((item, i)=><Option value={item.id} key={i}>{item.title}</Option>)}
						</Select>
						<FocusPic {...picListData} tipMessage={{isSame:0,message:this.state.tipArr}} onUpdateData={this.updateDataRecemened.bind(this)}/>
					</Tabpane>
				</Tabs>
				</div>
		)
	...
	changePics(index, field, value,type,width,height)
	{
		...
		if(this.expanded){
			...
			{
				ReactDOM.render(
							<FocusPic {...this.state.picListData} tipMessage={{isSame:0,message:this.state.tipArr}} onUpdateData={this.updateDataRecemened.bind(this,width,height)}/>,
							container
						);
			}
			else
			{
				let tip="请上传宽/高比例为"+width+"/"+height+"的图";
				ReactDOM.render(
							<FocusPic {...this.state.picListData} tipMessage={{isSame:1,message:[tip]}} onUpdateData={this.updateData.bind(this,width,height)}/>,
							container
						);
			}
