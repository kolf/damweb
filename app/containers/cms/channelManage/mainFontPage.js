...
import {channelCfg, scrollList, picList, deletePic, tagData, tagPicList,} 		  from "app/action_creators/cms_action_creator";
...
export default class FontPage extends Component {
	...
	render () {
		const {picListData, scrollList, alert} = this.state;
		...
		return (<div>
			...
			<Tabs type="card" onChange={this.tabChange.bind(this)}>
				<Tabpane tab="焦点图设置" key="1">
					<FocusPic {...picListData} tipMessage={{isSame:1,message:["请上传宽/高比例为1000/416的图"]}} onUpdateData={this.updateData.bind(this,1000,416)} addData={this.addData.bind(this)} deleteData={this.deleteData.bind(this)} sortAble={true}/>
				</Tabpane>
				...
				<Tabpane tab="热点活动" key="4">
					<FocusPic {...picListData} tipMessage={{isSame:1,message:["请上传宽/高比例为390/180的图"]}} onUpdateData={this.updateData.bind(this,390,180)} addData={this.addData.bind(this)} deleteData={this.deleteData.bind(this)} sortAble={true}/>
				</Tabpane>
			</Tabs>
		</div>)
	};
...
updateDataRecemened(index, field, value){
	...
	changePics(index, field, value,type,width,height)
	{
		...
		if(this.expanded){
			...
			{
				ReactDOM.render(
							<FocusPic {...this.state.picListData} tipMessage={{isSame:0,message:this.state.tipArr}} onUpdateData={this.updateDataRecemened.bind(this)}/>,
							container
						);
			}
			else
